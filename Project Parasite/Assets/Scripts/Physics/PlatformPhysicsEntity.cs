...
public class PlatformPhysicsEntity : RaycastController
{
    ...
    void MovePassengers(float velocityY) {
        ...
        if (velocityY != 0 && directionY == 1) {
            for (int i = 0; i < VERTICAL_RAY_COUNT; i++) {
                ...
                if (hit && !movedPassengers.Contains(hit.transform)) {
                    ...
                    MovePassenger(hit.transform, pushY);
        ...
        // Player on top, platform moving downwards
            ...
            for (int i = 0; i < VERTICAL_RAY_COUNT; i++) {
                ...
                if (hit && !movedPassengers.Contains(hit.transform)) {
                    float pushY = velocityY - (hit.distance - SKIN_WIDTH) * directionY;
                    MovePassenger(hit.transform, pushY);
                    movedPassengers.Add(hit.transform);
                }
            }
    ...
    private void MovePassenger(Transform colliderTransform, float pushY) {
        Character passenger = colliderTransform.GetComponentInParent<Character>();
        passenger.Move(Vector2.up * pushY);
    }
